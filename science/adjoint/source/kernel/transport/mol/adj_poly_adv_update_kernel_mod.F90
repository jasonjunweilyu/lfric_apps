...
!> @brief Line-by-line adjoint of poly_adv_update kernel. The wind field used is the
!>        linearisation state wind.
...
type, public, extends(kernel_type) :: adj_poly_adv_update_kernel_type
  type(arg_type) :: meta_args(5) = (/ &
    ...
    arg_type(GH_FIELD, GH_REAL, GH_READ, ANY_DISCONTINUOUS_SPACE_1, STENCIL(CROSS2D)), & ! dummy_md
...
subroutine adj_poly_adv_update_code( nlayers,        &
                                     advective,      &
                                     reconstruction, &
                                     dummy_md,       &
                                     smap_md_size,   &
  ...
  k = 0
  uv(1,k) = 0.25_r_tran * wind(map_w2(1)) + 0.25_r_tran * wind(map_w2(3))
  uv(2,k) = 0.25_r_tran * wind(map_w2(2)) + 0.25_r_tran * wind(map_w2(4))
  ...
  do k = 1, nlayers - 1, 1
    uv(1,k) = 0.25_r_tran * wind(k + map_w2(1)) + 0.25_r_tran * wind(k + map_w2(3)) + &
              0.25_r_tran * wind(k + map_w2(1) - 1) + 0.25_r_tran * wind(k + map_w2(3) - 1)
    uv(2,k) = 0.25_r_tran * wind(k + map_w2(2)) + 0.25_r_tran * wind(k + map_w2(4)) + &
              0.25_r_tran * wind(k + map_w2(2) - 1) + 0.25_r_tran * wind(k + map_w2(4) - 1)
  ...
  k = nlayers
  uv(1,k) = 0.25_r_tran * wind(k + map_w2(1) - 1) + 0.25_r_tran * wind(k + map_w2(3) - 1)
  uv(2,k) = 0.25_r_tran * wind(k + map_w2(2) - 1) + 0.25_r_tran * wind(k + map_w2(4) - 1)
